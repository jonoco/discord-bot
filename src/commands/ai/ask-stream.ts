...
const data = new SlashCommandBuilder()
  .setName('stream')
  .setDescription('Ask the bot a question with a stream')
...
async function execute(interaction: CommandInteraction) {
  ...
  try {
    const echoFragment = async (msg: string) => {
      const questionEcho = `> _${question}_\n`;
      await interaction.editReply(`${questionEcho}${msg}`);
    };

    const { error: questionError } = await askQuestionStream(
      question,
      echoFragment
    );
